...
def check_existing_color(guess, secret_combination):
    ...
    for index, caractere in enumerate(guess) :
        # print(f"caractere n°{index+1}")
        if caractere in secret_combination:
            if check_right_place(caractere, index, secret_combination): 
                exist_list.append('!')
                # print("color is in right place secret combination")
            else:
                exist_list.append('?')
                # print("color exist in secret combination")
        else: 
            exist_list.append('-')
            # print("color do not exist in secret combination")
